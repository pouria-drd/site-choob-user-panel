      ...
      <table className="divide-y divide-sc-gray-normal rounded-lg bg-sc-purple-normal border-b w-full r2l">
        ...
        <thead className=" rounded-lg">
          <tr>
            ...
            {data.headers.map((header, index) => (
              <th key={index} className="text-center px-2 py-4">
                {header}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-gray-200">
          {data.rows.map((row, rowIndex) => (
            <tr key={rowIndex} className="even:bg-gray-50 odd:bg-white p-4">
              {addIndex && (
                <td className="text-center overscroll-auto p-4 w-20">
                  {rowIndex + 1}
                </td>
              )}
