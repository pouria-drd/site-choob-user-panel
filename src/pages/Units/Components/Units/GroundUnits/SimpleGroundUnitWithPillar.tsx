...
function SimpleGroundUnitWithPillar({ projectId }: { projectId: string }) {
    ...
    const [dto, setDTO] = useState<SimpleGroundUnitWithPillarDTO>({ depth: 0, width: 0, height: 0, pillarDepth: 0, pillarWidth: 0, hasHiddenHandle: false, hiddenHandleTopGap: 0, shelfCount: 0, legColor: { colorName: 'رنگ 1' }, doors: [] });
    ...
    const handleInputChange = (fieldName: keyof SimpleGroundUnitWithPillarDTO, value: number | SimpleColorDTO) => {
        setDTO((prevDTO) => {
    ...
    const calculate = async () => {
        ...
        try {
            ...
            var result = await unitProjectService.CalculatedSimpleGroundUnitWithPillar<any>(dtoToSend);
    ...
    return (
        <div className="flex flex-col gap-2 r2l font-peyda  p-2  ">
            <h2 className="text-lg md:text-xl text-right font-semibold">یونیت زمینی ساده کنار ستون</h2>
...
export default SimpleGroundUnitWithPillar;
