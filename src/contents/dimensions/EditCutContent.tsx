...
const EditCutContent = ({
  dimension,
  onUpdate,
}: {
  dimension: DimensionDetailModel;
  onUpdate: () => void;
}) => {
  ...
  const [selectedWoodSheetDimensionID, setSelectedWoodSheetDimensionID] =
    useState(0);
  ...
  const handleSaveButtonClick = async () => {
    ...
    try {
      const result = await dimensionService.UpdateCutDimension<any>(
        formData,
        dimension.id
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await dimensionService.GetWoodSheetDimensions<
          WoodSheetDimension[]
        >();
        ...
        const relatedDimensionOption = newOptions.find(
          (op) => op.value === dimension.woodSheetDimensionsId.toString()
        );
        console.log(relatedDimensionOption);
